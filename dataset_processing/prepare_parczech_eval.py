...
# Example usage:
# python ./dataset_processing/prepare_parczech_eval.py \
#     --input_folder "/home/m/F5-TTS/audio_playground/parczech-3.0-asr-context.test/parczech-3.0-asr-context.test" \
#     --output_folder "/home/m/F5-TTS/audio_playground/cz/to_generate" \
#     --max_duration_sec 12 \
#     --target_minutes 10
...
def main():
    ...
    # 1. Obtain all .prt file paths (recursively)
    prt_files = list(input_folder.rglob("*.prt"))
    print(f"Found {len(prt_files)} .prt files in {input_folder}")
    ...
    for prt_file in prt_files:
        ...
        audio_file = prt_file.with_suffix(".wav")
        ...
        valid_files_dict[file_stem] = [str(prt_file.with_suffix("")), duration_min]
    
    print(f"After filtering by file name, {len(valid_files_dict)}/{len(prt_files)} files remain.")
    ...
    print(f"After filtering recordings above {args.max_duration_sec} sec, {len(filtered_files)}/{len(prt_files)} files remain.")
    ...
    print("\n=== Statistics ===")
    print(f"Total .prt files found: {len(prt_files)}")
